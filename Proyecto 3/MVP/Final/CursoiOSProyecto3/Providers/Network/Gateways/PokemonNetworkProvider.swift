...
class PokemonNetworkProvider: BaseNetworkProvider, PokemonProviderContract {
    ...
    func getPokemons() -> Promise<[Pokemon]> {
        var detailPagePromise: [Promise<Pokemon>] = []
        ...
        return Promise<[Pokemon]> { promise in
            ...
            self.getInitialPage().done { initialPage in
                ...
                when(resolved: detailPagePromise).done { results in
                    var fullfilledPromiseValues: [Pokemon] = []
    ...
    private func getPokemonDetail(fullURLString: String) -> Promise<Pokemon> {
        return Promise<Pokemon> { promise in
            sessionManager.request(fullURLString).responseJSON { response in
                ...
                if let pokemon = try? Pokemon(JSON: detailPageData) {
                    promise.fulfill(pokemon)
                } else {
                    promise.reject(PokemonNetworkError.detailPageLoadError)
                }
            }
